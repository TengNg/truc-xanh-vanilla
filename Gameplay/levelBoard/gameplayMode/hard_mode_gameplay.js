...
const questionImageData = [
   { imgSrc: "../../images/background_image/9.jpg", name: "ngựa non háu đá" },
   { imgSrc: "../../images/background_image/6.jpg", name: "giận cá chém thớt" },
   { imgSrc: "../../images/background_image/11.jpg", name: "bắt cá hai tay" },
   { imgSrc: "../../images/background_image/10.jpg", name: "trèo cao té đau" },
   { imgSrc: "../../images/background_image/8.jpg", name: "ăn quả nhớ kẻ trồng cây" },
   { imgSrc: "../../images/background_image/12.jpg", name: "ra ngô ra khoai" },
...
submitQuestionButton.addEventListener('click', function () {
   ...
   if (answerQuestionInput.value === removeAccents(randomImage.name)) {
      showQuestionButton.childNodes[1].classList = "fas fa-check-circle"
      showQuestionButton.style.pointerEvents = 'none'
      showQuestionButton.style.color = 'green'
      showQuestionButton.style.borderColor = 'green'
      isCorrectAnswer = true
      if(isFoundAllCards === true)
         WINNING()
   }
   answerQuestionInput.value = ""
})
...
function WINNING() {
   ...
   // calculate point
   point = 30 + startingTime

   // set point per stage in localStorage
   localStorage.setItem(`level3_stage${currentStage}_score`, point)

   // display point per stage
   let stored_point1 = parseInt(JSON.parse(localStorage.getItem(`level3_stage1_score`))) || 0
   let stored_point2 = parseInt(JSON.parse(localStorage.getItem(`level3_stage2_score`))) || 0
   let stored_point3 = parseInt(JSON.parse(localStorage.getItem(`level3_stage3_score`))) || 0
   let total_point = 0
   if(currentStage == 1)
      ...
      storingPointButton.addEventListener('click', () => {
         const stage1_score = parseInt(JSON.parse(localStorage.getItem('level3_stage1_score')))
         const stage2_score = parseInt(JSON.parse(localStorage.getItem('level3_stage2_score')))
         const stage3_score = parseInt(JSON.parse(localStorage.getItem('level3_stage3_score')))
         const player_score = {
            score: stage1_score + stage2_score + stage3_score,
            name: playerNameInput.value
         }
         highScoresLevel3.push(player_score)
         highScoresLevel3.sort((a, b) => b.score - a.score)
         highScoresLevel3.splice(10)
         localStorage.setItem('highScoresLevel3', JSON.stringify(highScoresLevel3))
         storingPointDiv.style.display = 'none'
...
function restart() {
   ...
   showQuestionButton.style.pointerEvents = 'all'
   showQuestionButton.childNodes[1].classList = 'fas fa-question'
   showQuestionButton.style.color = 'black'
   showQuestionButton.style.borderColor = 'black'
   isCorrectAnswer = false
   isFoundAllCards = false
